...
class PocMemoryEnv:
    ...
    def __init__(self, step_size: float = 0.2, glob: bool = False, freeze: bool = False):
        ...
        upper = (
            max(3.0 * self._step_size, self._step_size, (num_steps + 1) * self._step_size)
            if not glob
            else 1
        )
    ...
    def step(self, action):
        ...
        if self._num_show_steps > self._step_count:
            ...
            self._position = np.round(self._position, 2)
            obs = np.asarray(
                [0.0, self._position, 0.0], dtype=np.float32
            )  # mask out goal information
        ...
        if done:
            info = {"success": success, "reward": sum(self._rewards), "length": len(self._rewards)}
        else:
            info = None
